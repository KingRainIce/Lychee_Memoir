...
type AnnotationWorkspaceProps = {
  ...
  onPlanarPickChange: (pick: PlanarBasemapPick) => void
}
...
export function AnnotationWorkspace({
  ...
  const resolvedUrl = useMemo(
    () => resolvePlanarBasemapUrl(campusId, planarPick),
    [campusId, planarPick],
  )
  const [data, setData] = useState<AnnotationsFile>(() => loadLocalAnnotations(resolvedUrl))
  ...
  useEffect(() => {
    ...
    setData(loadLocalAnnotations(resolvedUrl))
    setSelectedId(null)
    setImageLoaded(false)
  }, [resolvedUrl])

  /** 重新进入工作台时：若磁盘 annotations.json 比本地缓存更新，则拉取并写回 localStorage（仅当与当前底图一致） */
  useEffect(() => {
  ...
  return (
    <div className="annotate-shell">
      <header className="annotate-bar">
        ...
        <label className="annotate-bar__basemap">
          <span className="annotate-bar__basemap-label">平面图</span>
          <select
            className="annotate-bar__basemap-select"
            value={planarPick}
            onChange={(e) => onPlanarPickChange(e.target.value as PlanarBasemapPick)}
            aria-label="选择平面图底图"
          >
            <option value="campus">
              跟随校区（{campuses[campusId]?.name ?? campusId}）
            </option>
            {PLANAR_BASEMAP_OPTIONS.map((o) => (
