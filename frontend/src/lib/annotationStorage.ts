...
function keyedAnnotationsStorageKey(imageFile: string): string {
  return `${ANNOTATIONS_STORAGE_KEY}::${imageFile}`
}
import {
...
function parseAnnotationsRaw(raw: string, imageFile: string): AnnotationsFile | null {
  try {
...
/** 按底图路径分库存储；旧版单 key 会在首次匹配时迁移到分 key */
export function loadLocalAnnotations(imageFile = BASEMAP_IMAGE): AnnotationsFile {
  try {
    const k = keyedAnnotationsStorageKey(imageFile)
    const keyed = localStorage.getItem(k)
    if (keyed) {
      const parsed = parseAnnotationsRaw(keyed, imageFile)
      if (parsed) return parsed
    }
    const legacy = localStorage.getItem(ANNOTATIONS_STORAGE_KEY)
    if (legacy) {
      const parsed = parseAnnotationsRaw(legacy, imageFile)
      if (!parsed) return emptyFile(imageFile)
      const storedFor = parsed.imageFile || BASEMAP_IMAGE
      if (storedFor !== imageFile) return emptyFile(imageFile)
      saveLocalAnnotations(parsed)
...
export function saveLocalAnnotations(data: AnnotationsFile): void {
  ...
  const img = next.imageFile || BASEMAP_IMAGE
  localStorage.setItem(keyedAnnotationsStorageKey(img), JSON.stringify(next))
...
export async function hydrateAnnotationsFromPublicIfMissing(): Promise<void> {
  ...
  if (!pub) return
  const img = pub.imageFile?.startsWith('/') ? pub.imageFile : pub.imageFile ? `/${pub.imageFile}` : BASEMAP_IMAGE
  if (localStorage.getItem(keyedAnnotationsStorageKey(img)) !== null) return
  saveLocalAnnotations({ ...pub, imageFile: img })
}
