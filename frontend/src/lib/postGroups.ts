...
/**
 * 地图气泡只显示有图片的帖子（无图帖子展示在记忆流侧边栏）
 */
export function groupPostsWithImageForMap(posts: AlumniPost[]): PostLocationStack[] {
  return groupAlumniPostsByLocation(posts.filter((p) => p.imageUrl?.trim()))
}
