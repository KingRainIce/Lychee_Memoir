...
export function Timeline({ monthIndex, onMonthIndexChange, isLatest }: TimelineProps) {
  ...
  const labelText = useMemo(() => {
    if (isLatest) return '现在'
    const { year, month } = historicalIndexToYearMonth(monthIndex)
    ...
    <div className="timeline">
      <div className="timeline__inner">
        <div className="timeline__label">
          <span className="timeline__title">时间</span>
            ...
            {Array.from({ length: totalTicks }, (_, i) => {
              ...
              return (
                <div key={i} className="timeline__tick-wrap" style={{ width: TICK_STEP }}>
                  <button
                    type="button"
                    className={`timeline__tick ${major ? 'timeline__tick--major' : ''} ${i === monthIndex ? 'timeline__tick--current' : ''}`}
                    style={{
